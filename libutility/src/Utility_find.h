 ...
 *
 */
#ifndef LIB_UTILITY_FIND_H
#define LIB_UTILITY_FIND_H
 ...
 * Returns index of first occurence of ch on success or -2 if ch is not present
 * EINVAL: invalid args passed*/
int utility_bindex_of(int ch,void *buf, size_t len);
...
/* sprintfs a name for a value given a key and parallel arrays of names and values
 * Return  NULL on error setting errno on error
 * Returns on success, pointer to element of 'names' at same offset as  first occurence of 'key' in 'values'
 * EINVAL: invalid args passed
 * ERANGE: max is too large*/
